...
const imageModules = import.meta.glob<{ default: ImageMetadata }>(
	"/src/content/posts/**/assets/*.{jpeg,jpg,png,gif,webp,svg}",
	{
		eager: true,
	},
);
	...
	cleaned = cleaned.replace(
		...
		(match, content) => {
			...
			if (imgMatches && imgMatches.length > 1) {
				...
				const lightImg = imgMatches.find((img: string) =>
					img.includes("img-light"),
				);
...
async function processImagePaths(
	htmlContent: string,
	siteUrl: string,
	postId: string,
): Promise<string> {
	...
	const processedPosts = await Promise.all(
		posts.map(async (post) => {
			...
			try {
				...
				// 处理图片路径
				const processedContent = await processImagePaths(
					cleanedContent,
					siteUrl,
					post.id,
				);
